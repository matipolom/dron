...
int main(int argc, char **argv) {
 ...
 printf("timestep %f\n",timestep);  
  ...
  while (wb_robot_step(timestep) != -1) {
    if (wb_robot_get_time() > 3.0)
      break;
  }


  // Constants, empirically found.
  double k_vertical_thrust = 68.5;  // with this thrust, the drone lifts.
  const double k_vertical_offset = 0.2;   // Vertical offset where the robot actually targets to stabilize itself.
  const double k_vertical_p = 0.5;        // P constant of the vertical PID.
  const double k_vertical_i = 0.3;
  const double k_vertical_d = 3000.0; 
  const double k_roll_p = 50.0;           // P constant of the roll PID.
  const double k_pitch_p = 30.0;          // P constant of the pitch PID.
  const double k_roll_i =1.0;
  const double k_pitch_i =1.0;
  const double k_roll_d =1.0;
  const double k_pitch_d =1.0;
  double pre_clamped_difference_altitude=0;
  // Variables.
  double target_altitude = 0.5;  // The target altitude. Can be changed by the user.
  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    const double roll_input = k_roll_p * CLAMP(roll, -1.0, 1.0) + k_roll_d * roll_acceleration + roll_disturbance;
    const double pitch_input = k_pitch_p * CLAMP(pitch, -1.0, 1.0) + k_pitch_d * pitch_acceleration + pitch_disturbance;
    ...
    const double vertical_input = k_vertical_p * clamped_difference_altitude + k_vertical_i * clamped_difference_altitude * timestep + k_vertical_d * (clamped_difference_altitude-pre_clamped_difference_altitude)/timestep;
    pre_clamped_difference_altitude =clamped_difference_altitude ;
    ...
    printf("vertical_input: %f\n",vertical_input);
    printf("roll_ input: %f\n",roll_input);
    printf("pitch_ input:  %f\n",pitch_input);  
    printf("yaw_ input:  %f\n",yaw_input);  
    printf("altitude %f\n",altitude);    
    printf("target altitude: %f [m]\n", target_altitude);
